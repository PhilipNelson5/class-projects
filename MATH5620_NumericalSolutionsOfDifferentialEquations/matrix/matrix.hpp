...
/* Remove the nth Row  */
template <typename T, std::size_t M, std::size_t N>
Matrix<T, M - 1, N> removeRow(Matrix<T, M, N> a, unsigned int n)
{
  T matrix[M - 1][N];
  int loc = 0;
  for (auto i = 0u; i < M; ++i)
...
/* Remove the nth Col  */
template <typename T, std::size_t M, std::size_t N>
Matrix<T, M, N - 1> removeCol(Matrix<T, M, N> a, unsigned int n)
{
  T matrix[M][N - 1];
  for (auto i = 0u; i < M; ++i)
  {
...
template <typename T,
          ...
          std::size_t N,
          typename R = decltype(T() * U())>
Matrix<R, M, N> operator*(Matrix<T, M, N> a, U scalar)
{
  R matrix[M][N];
  for (auto i = 0u; i < M; ++i)
  {
...
template <typename T,
          ...
          std::size_t N,
          typename R = decltype(T() * U())>
Matrix<R, M, N> operator*(U scalar, Matrix<T, M, N> a)
{
  R matrix[M][N];
  for (auto i = 0u; i < M; ++i)
  {
...
/* Stream Insertion */
template <typename T, std::size_t M, std::size_t N>
std::ostream& operator<<(std::ostream& o, Matrix<T, M, N> m)
{
  for (auto i = 0u; i < M; ++i)
  {
    for (auto j = 0u; j < N; ++j)
