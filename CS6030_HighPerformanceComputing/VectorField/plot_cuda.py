...
row = 0; col = 0
ax = axes[row][col]
ax.set_title("Total Program Time (1-64 Threads per Block)")
ax.set_ylabel("Time (ms)")
ax.set_xlabel("Number of Threads per Block")
for x in np.flip(df['curves'].unique()):
    sns.lineplot(data=df[(df["curves"]==x)&(df['threads'] < 100)], x="threads", y="program_time", label=f"{x}", ax=ax)

row = 0; col = 1
ax = axes[row][col]
ax.set_title("Streamline Calculation Time (1-64 Threads per Block)")
ax.set_ylabel("Time (ms)")
ax.set_xlabel("Number of Threads per Block")
for x in np.flip(df['curves'].unique()):
    sns.lineplot(data=df[(df["curves"]==x)&(df['threads'] < 100)], x="threads", y="curve_time", label=f"{x}", ax=ax)

row = 1; col = 0
ax = axes[row][col]
ax.set_title("Total Program Time (1-1024 Threads per Block)")
ax.set_ylabel("Time (ms)")
ax.set_xlabel("Number of Threads per Block")
for x in np.flip(df['curves'].unique()):
...
row = 1; col = 1
ax = axes[row][col]
ax.set_title("Streamline Calculation Time (1-1024 Threads per Block)")
ax.set_ylabel("Time (ms)")
ax.set_xlabel("Number of Threads per Block")
for x in np.flip(df['curves'].unique()):
