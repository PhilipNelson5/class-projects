...
struct Args
{
    int nThreadsPerBlock, nCurves, maxCurveLen, imgScaleFactor, vfH, vfW;
    bool makeImage;
...
{
    cxxopts::Options options(argv[0], "Compute streamlines in a vector field on a GPU");
    options.add_options()
        ("h,help", "Show usage")
        ("n,num_threads_per_block", "Number of CUDA threads per block", cxxopts::value<int>()->default_value("1024"))
    ...
    try {
        ...
        const int nThreadsPerBlock = result["num_threads_per_block"].as<int>();
        ...
        return Args { nThreadsPerBlock, nCurves, maxCurveLen, imgScaleFactor, vfH, vfW, makeImage, inputFileName };
