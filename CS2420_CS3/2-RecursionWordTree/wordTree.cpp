...
std::shared_ptr<Node> LLTree::preorderBuildR(std::ifstream & fin, std::shared_ptr<Node> const parent)
{
	std::string word;
	int numChildren;
...
//Creates a message containing the keys of a tree in preorder with the size of the sub tree and number of children printed next to each node.
void LLTree::printTreeR(std::shared_ptr<Node> const curr, std::stringstream & ss, std::string ind)
{
	if(!curr) return;
